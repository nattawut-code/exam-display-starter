...
$("clockSize").value=settings.clock_size;$("imageSize").value=settings.image_size;$("clockValue").textContent=settings.clock_size;$("imageValue").textContent=settings.image_size;
$("clockPosition").value=settings.clock_position;$("imagePosition").value=settings.image_position;
$("showDate").value=String(settings.show_date);$("showClock").value=String(settings.show_clock);$("showSchedule").value=String(settings.show_schedule);$("showImage").value=String(settings.show_image);
...
const oldPath=settings.image_url?settings.image_url.split("/").pop():null;
const ext=(selectedFile.name.split(".").pop()||"jpg").toLowerCase(),path=`display-${Date.now()}.${ext}`;
...
if(oldPath)await sb.storage.from(STORAGE_BUCKET).remove([oldPath]); // ลบรูปเก่าออกจาก storage กันพื้นที่เต็ม
}
...
setStatus("✅ ลบรูปจากหน้าจอแล้ว");$("previewBox").textContent="ไม่มีรูป";
if(oldPath)await sb.storage.from(STORAGE_BUCKET).remove([oldPath]);
}
