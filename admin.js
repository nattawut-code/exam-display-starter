...
const defaults={clockSize:110,imageSize:90,imagePosition:"center",imageAlign:"center",imageOffsetX:0,imageOffsetY:0,imageFit:"contain",showDate:true,showClock:true,clockPosition:"bottom",clockAlign:"center",clockOffset:0,clockFont:"system",dateSize:36,dateOffset:0,slideshowSeconds:8,theme:"midnight"};
...
function setPreviewImage(url){previewUrl=url||"";const image=$("previewImage"),empty=$("previewEmpty");if(url){if(image.src!==url)image.src=url;image.hidden=false;empty.hidden=true}else{image.removeAttribute("src");image.hidden=true;empty.hidden=false}}
function activeSchedules(now=new Date()){return schedules.filter(row=>(!row.start_at||new Date(row.start_at)<=now)&&(!row.end_at||new Date(row.end_at)>=now)).sort((a,b)=>(a.sort_order??0)-(b.sort_order??0))}
function currentSlideIndex(active,now=new Date()){if(active.length<2)return 0;const seconds=Math.max(3,settings.slideshowSeconds||8);return Math.floor(now.getTime()/(seconds*1000))%active.length}
function durationText(milliseconds){const total=Math.max(0,Math.ceil(milliseconds/1000)),h=Math.floor(total/3600),m=Math.floor(total%3600/60),s=total%60;return [h,m,s].map(n=>String(n).padStart(2,"0")).join(":")}
function formatTime(iso){try{return new Date(iso).toLocaleString("th-TH",{dateStyle:"medium",timeStyle:"short"})}catch{return ""}}
function renderBroadcast(){
  const now=new Date(),active=activeSchedules(now),title=$("onAirTitle"),detail=$("onAirDetail"),label=$("countdownLabel"),value=$("countdownValue"),countdownDetail=$("countdownDetail"),previewLabel=$("previewScheduleLabel");
  const nextStart=schedules.filter(row=>row.start_at&&new Date(row.start_at)>now).sort((a,b)=>new Date(a.start_at)-new Date(b.start_at))[0];
  if(active.length){
    const row=active[currentSlideIndex(active,now)],name=row.title||"ชุดตารางสอบไม่มีชื่อ";
    title.textContent=name;detail.textContent=active.length>1?`กำลังสลับแสดง ${active.length} ชุด ตามลำดับที่ตั้งไว้`:`กำลังแสดงอยู่${row.end_at?` ถึง ${formatTime(row.end_at)}`:""}`;
    previewLabel.textContent=`กำลังแสดง: ${name}`;
    if(!selectedFile&&row.image_url)setPreviewImage(row.image_url);
    if(active.length>1){const period=Math.max(3,settings.slideshowSeconds||8)*1000,remaining=period-(now.getTime()%period);label.textContent="เปลี่ยนไปชุดถัดไปใน";value.textContent=durationText(remaining);countdownDetail.textContent=`สลับทุก ${Math.max(3,settings.slideshowSeconds||8)} วินาที`}
    else if(row.end_at){const remaining=new Date(row.end_at)-now;label.textContent="ชุดนี้จะสิ้นสุดใน";value.textContent=durationText(remaining);countdownDetail.textContent=`สิ้นสุด ${formatTime(row.end_at)}`}
    else if(nextStart){const remaining=new Date(nextStart.start_at)-now;label.textContent="ชุดถัดไปเริ่มใน";value.textContent=durationText(remaining);countdownDetail.textContent=nextStart.title||"ชุดตารางสอบไม่มีชื่อ"}
    else{label.textContent="สถานะ";value.textContent="แสดงต่อเนื่อง";countdownDetail.textContent="ยังไม่ได้กำหนดเวลาสิ้นสุด"}
  }else if(nextStart){
    title.textContent="ยังไม่มีตารางกำลังออกอากาศ";detail.textContent="หน้าจอจะแสดงรูปชุดหลักจนกว่าจะถึงเวลาเริ่ม";previewLabel.textContent="ตัวอย่าง: รูปชุดหลัก";label.textContent="ชุดถัดไปเริ่มใน";value.textContent=durationText(new Date(nextStart.start_at)-now);countdownDetail.textContent=`${nextStart.title||"ชุดตารางสอบไม่มีชื่อ"} • ${formatTime(nextStart.start_at)}`;
  }else{title.textContent="ยังไม่มีตารางกำลังออกอากาศ";detail.textContent="เพิ่มชุดตารางสอบและกำหนดเวลาได้จากด้านล่าง";previewLabel.textContent="ตัวอย่าง: รูปชุดหลัก";label.textContent="สถานะ";value.textContent="รอการตั้งค่า";countdownDetail.textContent="ไม่มีชุดที่กำหนดเวลาเริ่มไว้"}
}
function syncControls(){
  $("clockSize").value=settings.clockSize;$("imageSize").value=settings.imageSize;$("imageOffsetX").value=settings.imageOffsetX;$("imageOffsetY").value=settings.imageOffsetY;$("clockOffset").value=settings.clockOffset;$("clockFont").value=settings.clockFont;$("dateSize").value=settings.dateSize;$("dateOffset").value=settings.dateOffset;$("slideshowSeconds").value=settings.slideshowSeconds;$("clockValue").value=settings.clockSize;$("imageValue").value=settings.imageSize;$("imageOffsetXValue").value=settings.imageOffsetX;$("imageOffsetYValue").value=settings.imageOffsetY;$("clockOffsetValue").value=settings.clockOffset;$("dateSizeValue").value=settings.dateSize;$("dateOffsetValue").value=settings.dateOffset;$("slideshowSecondsValue").value=settings.slideshowSeconds;
...
function renderPreview(){
  ...
  screen.className=`preview-screen preview-theme-${settings.theme} clock-${settings.clockPosition} clock-align-${settings.clockAlign}`;clock.style.fontSize=`${Math.max(18,settings.clockSize*.25)}px`;clock.style.transform=`translateY(${settings.clockOffset*.25}px)`;wrap.style.justifyContent={left:"flex-start",center:"center",right:"flex-end"}[settings.imageAlign]||"center";$("previewImage").style.objectFit=settings.imageFit||"contain";$("previewImage").style.transform=`translate(${(settings.imageOffsetX||0)*.25}px,${(settings.imageOffsetY||0)*.25}px)`;clock.style.fontFamily=fontFamily(settings.clockFont);date.style.fontSize=`${Math.max(9,settings.dateSize*.25)}px`;date.style.transform=`translateY(${(settings.dateOffset||0)*.25}px)`;clock.hidden=!settings.showClock;date.hidden=!settings.showDate;wrap.style.alignItems={top:"flex-start",center:"center",bottom:"flex-end"}[settings.imagePosition];$("previewImage").style.maxWidth=`${settings.imageSize}%`;
}
...
async function deleteMainImage(){if(!sb)return setStatus("⚠️ ต้องเข้าสู่ระบบก่อน");if(!confirm("ลบรูปตารางสอบชุดหลักหรือไม่?"))return;try{const {data:row,error}=await sb.from("display_settings").select("image_storage_path,image_url").eq("id",1).maybeSingle();if(error)throw error;let path=row?.image_storage_path||"";if(!path&&row?.image_url){const marker="/object/public/"+STORAGE_BUCKET+"/";const i=row.image_url.indexOf(marker);if(i>=0)path=decodeURIComponent(row.image_url.slice(i+marker.length).split("?")[0])}if(path)await sb.storage.from(STORAGE_BUCKET).remove([path]);const {error:dbError}=await sb.from("display_settings").upsert({id:1,image_url:null,image_storage_path:null});if(dbError)throw dbError;selectedFile=null;previewUrl="";$("imageFile").value="";setPreviewImage("");localStorage.removeItem(IMAGE_KEY);setStatus("✅ ลบรูปชุดหลักแล้ว")}catch(e){setStatus(`⚠️ ลบรูปไม่สำเร็จ: ${e.message||"ลองใหม่อีกครั้ง"}`)}}
$("signIn").onclick=signIn;$("signOut").onclick=signOut;$("saveSettings").onclick=saveSettings;$("publish").onclick=publish;$("deleteMainImage").onclick=deleteMainImage;setInterval(()=>{renderPreview();renderBroadcast()},1000);load();
