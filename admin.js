...
async function publish(){await saveSettings();if(!selectedFile){setStatus("✅ เผยแพร่การตั้งค่าแล้ว");return}if(!sb){saveLocal();setStatus("✅ บันทึกรูปในเบราว์เซอร์แล้ว");return}try{setStatus("กำลังปรับขนาดและอัปโหลดรูป...");const resized=await resizeImage(selectedFile);const ext=(selectedFile.name.split(".").pop()||"jpg").toLowerCase()==="png"?"png":"jpg",path=`display-${Date.now()}.${ext}`;const {error}=await sb.storage.from(STORAGE_BUCKET).upload(path,resized,{contentType:resized.type,upsert:false});if(error)throw error;const {data}=sb.storage.from(STORAGE_BUCKET).getPublicUrl(path);const {error:dbError}=await sb.from("display_settings").upsert({id:1,image_url:data.publicUrl,image_storage_path:path});if(dbError)throw dbError;setPreviewImage(data.publicUrl);setStatus("✅ อัปโหลดและเผยแพร่แล้ว")}catch(e){setStatus(`⚠️ เก็บรูปไว้ในเครื่องแล้ว: ${e.message||"อัปโหลดไม่สำเร็จ"}`);saveLocal()}}
...
$("signIn").onclick=signIn;$("signOut").onclick=signOut;$("saveSettings").onclick=saveSettings;$("publish").onclick=publish;$("deleteMainImage").onclick=deleteMainImage;setInterval(renderPreview,1000);load();
